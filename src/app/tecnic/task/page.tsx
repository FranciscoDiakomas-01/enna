...
import {
  createTaskSave,
  getAllTaskByUserId,
  getAllTaskByUserIdSearch,
  getAllTaskByUserStatus,
} from "@/services/task";
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    async function getTaskByStatus() {
      const data = await getAllTaskByUserStatus(String(page), status);
      setTasks(data?.data);
      setTimeout(() => {
        setLoading(false);
      }, 2000);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    async function get() {
      const data = await getAllTaskByUserId(String(page));
      setTasks(data?.data);
      setPage(data?.page);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    async function get() {
      const data = await getAllTaskByUserIdSearch(String(page), search);
      setTasks(data?.data);
      setPage(data?.page);
      setLastPage(data?.lastPage);
      setTimeout(() => {
  ...
  async function createTask(e: React.FormEvent<HTMLFormElement>) {
    ...
    const owerId = String(localStorage.getItem("userid")) as string;
